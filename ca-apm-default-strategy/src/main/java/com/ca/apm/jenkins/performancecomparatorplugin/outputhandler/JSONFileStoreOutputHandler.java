...
/**
 * An implementation of Output-Strategy which takes the selected comparison-strategy results and
 * prepare a JSON output and stores into the configured output.directory If output.directory is not
 * provided, it will flush to Jenkins Current Build workspace folder
 *
...
public class JSONFileStoreOutputHandler implements OutputHandler<StrategyResult> {
  ...
  public void publishOutput(List<StrategyResult> comparisonStrategyResults)
    ...
    for (StrategyResult<?> strategyResult : comparisonStrategyResults) {
      String json =
          DataFormatHelper.generateJSONOutputForStrategy((StrategyResult<?>) strategyResult);
      JSONObject strategyJson = new JSONObject(json);
      resultsArray.put(strategyJson);
    }
    FileHelper.exportOutputToFile(
        outputPath, "json-file-store-output-handler.json", outputObject.toString(1));
  }
}
