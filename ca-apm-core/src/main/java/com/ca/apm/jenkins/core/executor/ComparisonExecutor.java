	...
	void execute() throws BuildExecutionException {
		...
		for (String comparisonStrategy : comparisonStrategies.keySet()) {
			...
			String comparatorClass = strategyConfiguration
					.getPropertyValue(comparisonStrategy + "."+Constants.comparatorClasssName);
			...
			try {
				...
				Method compareMethod = pluginClass.getDeclaredMethod(Constants.comparatorExecuteMethod, BuildInfo.class, BuildInfo.class);
				...
				if (comparisonMetadata.getStrategiesInfo().isComparisonStrategyNonMapped(comparisonStrategy)) {
					JenkinsPlugInLogger.warning(comparisonStrategy
							+ " is not mapped to any output-handler, hence this output will not be used any where");
					builder.append("     " + comparisonStrategy
							+ " is not mapped to any output-handler, hence this output will not be used any where")
							.append(Constants.NewLine);
				}
			} catch (ClassNotFoundException e) {
				JenkinsPlugInLogger.severe(comparatorClass + " could not be found which is configured  for "
						+ comparisonStrategy + " strategy, hence ignoring this comparison", e);
				builder.append("     ").append("Error :" + comparisonStrategy + " execution failed" + e.getMessage())
						.append(Constants.NewLine);
				continue;
			} catch (IllegalAccessException e) {
				JenkinsPlugInLogger
						.severe("Error in executing comparison strategy ->" + comparisonStrategy + e.getMessage(), e);
				builder.append("     ").append("Error :" + comparisonStrategy + " execution failed"  + e.getMessage())
						.append(Constants.NewLine);
				continue;
				...
				if(e.getTargetException().getMessage().toString().contains("Connection refused")){
					...
					if(e.getTargetException().getMessage().contains(apmConnectionInfo.getEmURL().substring(apmHostNameIndex,apmConnectionInfo.getEmURL().lastIndexOf(':') ))){
				...
				JenkinsPlugInLogger.severe("Error in executing comparison strategy ->" + comparisonStrategy + " with ->"
				...
				JenkinsPlugInLogger.severe(
						"Error in executing comparison strategy ->" + comparisonStrategy + " with ->" + e.getMessage(),
						e);
				builder.append("     ").append("Error :" + comparisonStrategy + " execution failed"  + e.getMessage())
						.append(Constants.NewLine);
				continue;
				...
				JenkinsPlugInLogger.severe(
						"Error in executing comparison strategy ->" + comparisonStrategy + " with ->" + e.getMessage(),
						e);
				builder.append("     ").append("Error :" + comparisonStrategy + " execution failed")
						.append(Constants.NewLine);
				continue;
			}
			builder.append("     ").append(comparisonStrategy + " executed successfully").append(Constants.NewLine);
		}
