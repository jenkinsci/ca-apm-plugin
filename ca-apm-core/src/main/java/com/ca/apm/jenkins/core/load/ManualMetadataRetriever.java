...
public class ManualMetadataRetriever implements LoadRunnerMetadataRetriever {
  ...
  public ManualMetadataRetriever(LoadRunnerMetadata loadRunnerMetadata)
  ...
  /**
   * This method is used to fetch metadata based on given current and benchmark start and end time
   * for manual load runner.
   */
  public void fetchExtraMetadata() throws BuildValidationException {
    ...
    String currentBuildStartTime =
        loadRunnerMetadata.getLoadRunnerPropertyValue(loadRunnerName + ".currentrunloadstarttime");
    String currentBuildEndTime =
        loadRunnerMetadata.getLoadRunnerPropertyValue(loadRunnerName + ".currentrunloadendtime");
    String benchMarkBuildStartTime =
        loadRunnerMetadata.getLoadRunnerPropertyValue(
            loadRunnerName + ".benchmarkrunloadstarttime");
    String benchMarkBuildEndTime =
        loadRunnerMetadata.getLoadRunnerPropertyValue(loadRunnerName + ".benchmarkrunloadendtime");

    if (currentBuildEndTime == null
        || currentBuildEndTime.isEmpty()
        || currentBuildStartTime == null
        || currentBuildStartTime.isEmpty()
        || benchMarkBuildEndTime == null
        || benchMarkBuildEndTime.isEmpty()
        || benchMarkBuildStartTime == null
        || benchMarkBuildStartTime.isEmpty()) {
