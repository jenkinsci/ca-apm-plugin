...
/**
 * This class is to fetch jmeter run metadata when a file is provided at
 * jmeter's workspace job directory.
 * 
 */
public class JmeterMetadataRetriever implements LoadRunnerMetadataRetriever {
	...
	/**
	 * This method is used to fetch jmeter metadata when the output file of
	 * jmeter is provided for the current run and benchmark run as well.
	 * 
	 */
	...
	public void fetchExtraMetadata() throws BuildValidationException {
		...
		this.jmeterCurrentRunOutputFile = loadRunnerMetadata
				.getLoadRunnerPropertyValue(loadRunnerName + ".currentrunoutputfile");

		this.jmeterBenchmarkRunOutputFile = loadRunnerMetadata
				.getLoadRunnerPropertyValue(loadRunnerName + ".benchmarkrunoutputfile");
	...
	/**
	 * This method is used to read jmeter output file . Throws
	...
	private void readLoadGenFile() throws BuildValidationException {
		...
		if (fileType.equalsIgnoreCase("csv")) {
			...
			jmeterCurrentRunOutputFile = jenkinsInfo.getBuildWorkSpaceFolder() + File.separator
					+ jenkinsInfo.getJobName() + File.separator + jenkinsInfo.getCurrentBuildNumber() + File.separator
			...
			jmeterBenchmarkRunOutputFile = jenkinsInfo.getBuildWorkSpaceFolder() + File.separator
					+ jenkinsInfo.getJobName() + File.separator + loadRunnerMetadata.getBenchMarkBuildNumber()
					+ File.separator + jmeterBenchmarkRunOutputFile;
