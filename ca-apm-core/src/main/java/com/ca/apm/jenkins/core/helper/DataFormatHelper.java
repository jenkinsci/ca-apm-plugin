...
public class DataFormatHelper {
	...
	public static String getHTMLTextOutput(OutputConfiguration outputConfiguration,
			List<StrategyResult> strategyResults) {

		List<StrategyResult<DefaultStrategyResult>> defaultStrategyResults = new LinkedList<StrategyResult<DefaultStrategyResult>>();
		...
		if (defaultStrategyResults.isEmpty()) {
			JenkinsPlugInLogger
					.severe("No Default Strategy Result Found in the handler, hence the output cannot be generated");
			return null;
		}
		...
		context.put("benchmarkbuildnumber", outputConfiguration.getCommonPropertyValue(Constants.jenkinsBenchMarkBuild));
