...
public class IOUtility {
  ...
  public Class<?> findClass(String className) throws ClassNotFoundException {
    ...
    try {
      ...
      try {
        if (extensionsDirectory != null) pluginClass = Class.forName(className, true, classLoader);
  ...
  public void extractZipFromClassPath(String zipFileName, File targetFolder) {
    ...
    ZipInputStream zipInputStream =
        new ZipInputStream(classLoader.getResourceAsStream(zipFileName));
    ZipEntry entry = null;
    try {
  ...
  public void loadExtensionsLibraries() {
    ...
    if (extensionsDirectory == null) {
      JenkinsPlugInLogger.info(
          "No jar file(s) found in the extensions directory specified in configuration");
      return;
    }
