...
import com.ca.apm.jenkins.api.exception.DecodeExceededDataLength;
...
public class CommonEncryptionProvider {
	...
	public String encrypt(String str) throws InvalidKeyException, NoSuchAlgorithmException, InvalidKeySpecException,
	...
	public String decrypt(String str) throws InvalidKeyException, NoSuchAlgorithmException, InvalidKeySpecException,
			NoSuchPaddingException, InvalidAlgorithmParameterException, BadPaddingException {
		if (!encryptionEnabled)
			return str;
		try {
			byte[] bytes = decod(str);
	...
	private byte[] decod(String str) {
		byte[] bytes = new byte[0];
		try {
			bytes = decode(str.toCharArray());

		} catch (Exception e) {
			JenkinsPlugInLogger.log(Level.SEVERE, e.getMessage());
		}
		return bytes;
	}
	 ...
	 *
	 * 
	 * @throws DecodeExceededDataLength
	 */
	protected byte[] decode(char[] data) throws DecodeExceededDataLength {
		...
		if (index != out.length) {
			throw new DecodeExceededDataLength(
	...
	public String getDecryptedPassword(String password) throws InvalidKeyException, NoSuchAlgorithmException,
