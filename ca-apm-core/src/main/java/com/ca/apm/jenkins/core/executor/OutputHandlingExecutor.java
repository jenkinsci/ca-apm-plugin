...
/**
 * The executor for running the configured output-handlers. This takes care of loading the basic and
 * custom-created output-handlers from the extensions path and executes the output-handler step.
 *
...
public class OutputHandlingExecutor {
  ...
  void execute(OutputConfiguration outputConfiguration, boolean isFailToBuild) {
    JenkinsPlugInLogger.printLogOnConsole(
        1, "Starting the output handling phase" + Constants.NewLine);
    ...
    Map<String, OutputHandlerConfiguration> outputStrategies =
        strategiesInfo.getOutputHandlersInfo();
    ...
    for (String outputHandler : outputStrategies.keySet()) {
      ...
      try {
        ...
        Method setConfigurationMethod =
            pluginClass.getDeclaredMethod(
                Constants.outputHandlerConfigMethod, OutputConfiguration.class);
        outputConfiguration.setHandlerSpecificProperties(
            outputStrategies.get(outputHandler).getProperties());
        outputConfiguration.addToCommonProperties(
            "buildStatus", isFailToBuild == true ? "FAILURE" : "SUCCESS");
        outputConfiguration.addToCommonProperties(
            "frequency",
            String.valueOf(
                comparisonMetadata
        ...
        Set<String> comparisonStrategies =
            strategiesInfo.getMappedComparisonStrategies(outputHandler);
        List<StrategyResult<?>> selectedStrategyResult =
            comparisonResult.getSelectiveComparisonResults(outputHandler, comparisonStrategies);
        if (selectedStrategyResult == null) {
          JenkinsPlugInLogger.warning(
              "No results obtained from the selective comparison strategy for "
                  + outputHandler
                  + Constants.NewLine);
          JenkinsPlugInLogger.printLogOnConsole(
              3,
        ...
        Method publishMethod =
            pluginClass.getDeclaredMethod(Constants.outputHandlerExecuteMethod, List.class);
        ...
        builder
            .append("     ")
            .append(outputHandler + " output handler execution failed")
            .append(Constants.NewLine);
        continue;
      } catch (IllegalAccessException e) {
        JenkinsPlugInLogger.severe(
            "Error in executing Output strategy ->" + outputHandler + " with ->" + e.getMessage(),
            e);
        builder
            .append("     ")
            .append(outputHandler + " output handler execution failed")
        ...
        JenkinsPlugInLogger.severe(
            "Error in executing Output strategy ->"
                + outputHandler
                + " with ->"
                + e.getMessage()
                + Constants.NewLine,
      ...
      builder
          .append("     ")
          .append(outputHandler + " output handler executed successfully")
          .append(Constants.NewLine);
    }
    JenkinsPlugInLogger.printLogOnConsole(
        3,
        "Total number of output handlers executed are "
            + outputStrategies.size()
            + Constants.NewLine
            + builder.toString());
