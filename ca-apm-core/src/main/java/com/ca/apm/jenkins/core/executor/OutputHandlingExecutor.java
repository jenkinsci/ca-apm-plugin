	...
	void execute(OutputConfiguration outputConfiguration, boolean isFailToBuild) {
		...
		for (Map.Entry<String, OutputHandlerConfiguration> outputHandlerEntry : outputStrategies.entrySet()) {
			...
			try {
				...
				Set<String> comparisonStrategies = strategiesInfo.getMappedComparisonStrategies(outputHandlerEntry.getKey());
				...
				continue;
			} catch (InvocationTargetException | IllegalAccessException | InstantiationException | NoSuchMethodException | SecurityException e) {
