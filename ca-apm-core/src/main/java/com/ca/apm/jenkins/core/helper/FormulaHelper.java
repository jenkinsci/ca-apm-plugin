...
/**
 * Helper utility to provide you the average, minimum, max values of the
 * performance data per metric_path
 *
 * <p>
 * It also provides you formula to do percentage based or threshold value based
 * comparison of metrics between the two given builds
 *
...
public class FormulaHelper {
	...
	/**
	 * Get the Time Slices group by metric path, this is useful to pass the
	 * value to output-handler
	 *
	 * @param buildPerformanceData
	 *            The buildPerformanceData object is retrieved using
	 *            MetricDataHelper.getMetricData using the specified arguments
	 * @return Map of Metric Path as key and value as list of TimeSlices of
	 *         performance values obtained from APM
	 */
	...
	/**
	 * Get the average values of each metric path of the given build's
	 * performance data
	 *
	 * @param buildPerformanceData
	 *            The entity you receive from MetricDataHelper on querying the
	 *            metrics
	 * @return Returns a Map with key = metricPath and value = the average of
	 *         the metric value
	 */
	public static Map<String, Double> getAverageValues(BuildPerformanceData buildPerformanceData) {
		if (buildPerformanceData == null) {
			JenkinsPlugInLogger
					.warning("Build Performance Metric data is empty, hence not applying the average formula");
			return null;
		}
		Collection<MetricPerformanceData> transactions = buildPerformanceData.getMetricData();
		Map<String, Double> buildAverageValues = new HashMap<>();
	...
	/**
	 * Get the maximum values of each metric path of the given build's
	 * performance data
	 *
	 * @param buildPerformanceData
	 *            The entity you receive from MetricDataHelper on querying the
	...
	/**
	 * Get the minimum values of each metric path of the given build's
	 * performance data
	 *
	 * @param buildPerformanceData
	 *            The entity you receive from MetricDataHelper on querying the
	 *            metrics
	 * @return Returns a Map with key =metricPath and value = the average of the
	 *         metric value
	 */
	...
	/**
	 * Get the percentage change in positive or negative value depending upon
	 * the values for any metric
	 *
	 * @param expectedValue
	 *            The metric value corresponding to bench-mark build
	 * @param actualValue
	 *            The metric value corresponding to current build
	 * @return Returns the percentage change positive or negative of the actual
	 *         value w.r.t. expected value
	 */
	...
	/**
	 * This helper method provides you comparison-result w.r.t. your one agent
	 * specifier configured in your strategies properties. This method does the
	 * comparison of your current build's performance with bench-mark build's
	 * performance of corresponding metric-path If the current build's metric
	 * path's value is threshold% greater than bench-mark build's performance,
	 * the metric record is considered to be slow performing Else it is
	 * considered as to be performing well
	 *
	 * @param thresholdPercentage
	 *            The percentage value defined by the user in the strategies
	 *            configuration
	 * @param benchMarkValues
	 *            The bench-mark performance data, which is average, minimum,
	 *            max or count, this can be created from other helpers like
	 *            getAverageValues, getMinValues, getMaxValues present in this
	 *            same helper class
	 * @param currentValues
	 *            Current performance values of Current Build, per metric-path
	...
	public static AgentComparisonResult thresholdPercentageBasedCrossBuildMetricPathComparison(
		...
		for (Map.Entry<String, Double> metricPathEntry : benchMarkValues.entrySet()) {
			double expectedValue = benchMarkValues.get(metricPathEntry.getKey());
			double actualValue = currentValues.get(metricPathEntry.getKey());
			...
			percentageChange = JenkinsPluginUtility.getDoubleFormattedToTwoDecimalPlaces(percentageChange);
			metricPathComparisonResult.setActualValue(actualValue);
			metricPathComparisonResult.setExpectedValue(expectedValue);
			metricPathComparisonResult.setMetricPath(metricPathEntry.getKey());
	 ...
	 * This helper method provides you comparison-result w.r.t. your one agent
	 * specifier configured in your strategies properties. This method does the
	 * comparison of your current build's performance with threshold value
	 * provided in the configuration. If the current build's metric path's value
	 * is greater than threshold value, the metric record is considered to be
	 * slow performing Else it is considered as to be performing well
	 *
	 * @param thresholdValue
	 *            The threshold value defined by the user in the strategies
	 *            configuration
	 * @param currentAverageValues
	 *            The current build's performance data, which is average,
	 *            minimum, max or count, this can be created from other helpers
	 *            like getAverageValues, getMinValues, getMaxValues present in
	 *            this same helper class
	 * @return Refer AgentComparisonResult
	 */
	public static AgentComparisonResult thresholdValueBasedCrossBuildMetricPathComparison(double thresholdValue,
			Map<String, Double> currentAverageValues) {
		AgentComparisonResult agentComparisonResult = new AgentComparisonResult();
		for (Map.Entry<String, Double> metricPathEntry : currentAverageValues.entrySet()) {
			double actualValue = currentAverageValues.get(metricPathEntry.getKey());
