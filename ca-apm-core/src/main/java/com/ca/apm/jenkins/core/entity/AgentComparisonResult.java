 ...
 * Comparison-strategy. This contains the list of successEntries and slowEntries
 *
 * <p>successEntries represent the comparison-result of those metric-paths whose performance was
 * considered to be under the benchmark or threshold-limits.
 *
 * <p>slowEntries represent the comparison-result of those metric-paths whose performance was
 * considered to be above the tolerance or threshold limits configured
 *
 * <p>With the help of two separate lists, we can easily find out the trouble causing components
 *
 * <p>By default, we have a few re-usable functions in FormulaHelper, you can use them and get this
 * entity generated for you. Also if you want to decide your own way to choose whether to tag a
 * comparison-result to be normal or alarming, you can do it your own way as well
 *
...
public class AgentComparisonResult {
  ...
  public String toString() {
    return "ComparisonResult [successEntries="
        + successEntries
        + ", slowEntries="
        + slowEntries
        + "]";
  ...
  public void attachEveryPointResult(
      double threshold, Map<String, List<TimeSliceValue>> currentTimeSliceValues) {
    if (!successEntries.isEmpty()) {
      for (MetricPathComparisonResult comparisonResult : successEntries) {
        List<TimeSliceValue> actualValues =
            currentTimeSliceValues.get(comparisonResult.getMetricPath());
        List<TimeSliceValue> expectedValues = new LinkedList<TimeSliceValue>();
        for (int i = 0; i < actualValues.size(); i++) {
          TimeSliceValue tsV =
              new TimeSliceValue(
                  threshold, threshold, threshold, 1, actualValues.get(i).getfrequency());
  ...
  public void attachEveryPointResult(
      Map<String, List<TimeSliceValue>> benchMarkTimeSliceValues,
    ...
    if (!successEntries.isEmpty()) {
      for (MetricPathComparisonResult comparisonResult : successEntries) {
        ...
        comparisonResult.setCurrentBuildTimeSliceValues(
            currentTimeSliceValues.get(comparisonResult.getMetricPath()));
      }
    }
    if (!slowEntries.isEmpty()) {
